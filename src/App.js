...
// --- ▽▽▽ AI新聞記事モーダル（ここから） ▽▽▽ ---
const NewspaperModal = ({ isOpen, onClose, article, isLoading }) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-stone-100 w-full max-w-2xl rounded-sm shadow-2xl overflow-hidden relative">
        {/* 閉じるボタン */}
        ...
        {isLoading ? (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-600 font-serif text-lg animate-pulse">
              AI記者が記事を執筆中...<br/>
              <span className="text-sm">（これには10秒ほどかかります）</span>
            </p>
          </div>
        ) : (
          <div className="p-6 md:p-8 font-serif">
            {/* 新聞ヘッダー */}
            <div className="border-b-4 border-double border-gray-800 mb-4 pb-2 text-center">
              <h2 className="text-xs md:text-sm text-gray-500 tracking-widest mb-1">SOFTBALL TIMES EXTRA</h2>
              <h1 className="text-3xl md:text-4xl font-black text-gray-900 leading-tight">{article.headline}</h1>
            </div>

            {/* 記事本文 */}
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                <p className="text-lg leading-relaxed text-justify text-gray-800 whitespace-pre-wrap">
                  <span className="text-3xl float-left mr-2 mt-[-6px] font-bold">【</span>
                  {article.content}
                </p>
              </div>
            </div>
            ...
            {/* フッター */}
            <div className="mt-8 pt-4 border-t border-gray-400 text-right text-xs text-gray-500">
              Generated by Gemini AI • 試合速報アプリ
            </div>
          </div>
        )}
      </div>
